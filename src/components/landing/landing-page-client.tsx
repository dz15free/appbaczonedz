...
export default function LandingPage({ latestPosts = [] }: { latestPosts?: BlogIndexEntry[] }) {
  const { settings: s, loaded } = useSiteSettings();
  const { user, loading: authLoading } = useAuth();
  const brandHref = authLoading ? "/" : user ? "/home" : "/";
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-[var(--bz-bg)] text-[var(--bz-text)] antialiased selection:bg-blue-500/30">
      ...
      {/* ═══════════════ TOOLS + GUIDES ═══════════════ */}
      <section className="bz-landing-tools px-5 py-16 sm:px-6 sm:py-24">
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 flex flex-col gap-3 sm:mb-12 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <span className="inline-flex rounded-full bg-primary/10 px-3 py-1 text-[11px] font-bold tracking-[0.16em] text-primary">أدوات عملية</span>
              <h2 className="mt-3 font-display text-2xl font-extrabold tracking-tight sm:text-3xl">كل ما تحتاجه في طريق البكالوريا</h2>
              <p className="mt-3 max-w-2xl text-[14px] leading-relaxed text-text-muted sm:text-[15px]">أدوات حقيقية تساعدك على الحساب والتنظيم والتدرب، مع أدلة وتخصصات قابلة للقراءة قبل اتخاذ القرار.</p>
            </div>
            <Link href="/tools" className="inline-flex items-center gap-2 text-sm font-extrabold text-primary hover:underline">استكشف الأدوات <FontAwesomeIcon icon={faArrowLeft} className="h-3 w-3" /></Link>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              { href: "/calculate", icon: faCalculator, title: "حساب معدل البكالوريا", desc: "احسب معدلك حسب شعبتك ومعاملاتك." },
              { href: "/tools/weighted-average", icon: faScaleBalanced, title: "المعدل الموزون", desc: "قارن نتيجتك مع شروط التوجيه بطريقة واضحة." },
              { href: "/tools/exam-simulator", icon: faClipboardCheck, title: "محاكاة الامتحان", desc: "ادخل إلى جلسة امتحان بوقت وحالة واضحة." },
              { href: "/tools/study-planner", icon: faCalendarDays, title: "برنامج المراجعة", desc: "أنشئ جدولاً يناسب أيامك وفتراتك." },
              { href: "/tools/planner", icon: faCalendarDays, title: "مخطط البكالوريا", desc: "نظّم المواد والمهام في ورقة قابلة للطباعة." },
              { href: "/specialties", icon: faGraduationCap, title: "دليل التخصصات", desc: "اقرأ ما تدرس وآفاق كل تخصص قبل الرغبات." },
            ].map((tool) => (
              <Link key={tool.href} href={tool.href} className="group rounded-2xl border border-border/60 bg-surface p-5 transition hover:-translate-y-1 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5">
                <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary transition group-hover:bg-primary group-hover:text-white"><FontAwesomeIcon icon={tool.icon} className="h-5 w-5" /></span>
                <h3 className="mt-4 font-display text-[15px] font-extrabold">{tool.title}</h3>
                <p className="mt-1.5 text-[13px] leading-relaxed text-text-muted">{tool.desc}</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      {/* ═══════════════ BLOG ═══════════════ */}
      <section className="bz-landing-blog bg-surface/60 px-5 py-16 sm:px-6 sm:py-24">
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 flex flex-col gap-3 sm:mb-12 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <span className="inline-flex rounded-full bg-primary/10 px-3 py-1 text-[11px] font-bold tracking-[0.16em] text-primary">من مدونة BacZone</span>
              <h2 className="mt-3 font-display text-2xl font-extrabold tracking-tight sm:text-3xl">محتوى تعليمي يرافقك فعلاً</h2>
              <p className="mt-3 max-w-2xl text-[14px] leading-relaxed text-text-muted sm:text-[15px]">أحدث المقالات المنشورة من لوحة المدونة، ببطاقات واضحة وروابط مباشرة للقراءة.</p>
            </div>
            <Link href="/blog" className="inline-flex items-center gap-2 text-sm font-extrabold text-primary hover:underline">عرض كل المقالات <FontAwesomeIcon icon={faArrowLeft} className="h-3 w-3" /></Link>
          </div>
          {latestPosts.length > 0 ? (
            <div className="grid gap-5 md:grid-cols-3">
              {latestPosts.slice(0, 3).map((post) => (
                <Link key={post.id} href={`/blog/${post.slug}`} className="group flex h-full flex-col overflow-hidden rounded-3xl border border-border/60 bg-[var(--bz-bg)] transition hover:-translate-y-1 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5">
                  {post.cover ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={post.cover} alt={post.coverAlt || post.title} width={720} height={380} loading="lazy" decoding="async" className="aspect-[720/380] w-full object-cover transition duration-500 group-hover:scale-[1.03]" />
                  ) : <div className="flex aspect-[720/380] items-center justify-center bg-gradient-to-br from-primary/15 to-emerald-500/10 text-primary"><FontAwesomeIcon icon={faNewspaper} className="h-10 w-10" /></div>}
                  <span className="flex flex-1 flex-col p-5">
                    <span className="text-[11px] font-extrabold text-primary">{post.labels?.[0] ? labelName(post.labels[0]) : "دليل BacZone"}</span>
                    <span className="mt-2 font-display text-[16px] font-extrabold leading-[1.55]">{post.title}</span>
                    {post.excerpt && <span className="mt-2 line-clamp-3 text-[13px] leading-[1.8] text-text-muted">{post.excerpt}</span>}
                    <span className="mt-auto pt-4 text-[11.5px] text-text-muted">{post.publishedAt ? new Intl.DateTimeFormat("ar-DZ", { year: "numeric", month: "long", day: "numeric" }).format(new Date(post.publishedAt)) : "منشور حديثاً"} · {post.readMinutes} دقائق</span>
                  </span>
                </Link>
              ))}
            </div>
          ) : (
            <div className="rounded-3xl border border-dashed border-border bg-[var(--bz-bg)] p-8 text-center text-sm text-text-muted">تُنشر المقالات هنا تلقائياً عند اعتمادها من لوحة المدونة. <Link href="/blog" className="font-bold text-primary hover:underline">تصفح المدونة</Link></div>
          )}
        </div>
      </section>
