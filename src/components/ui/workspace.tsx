...
/* ════════════════════════════════════════════════════════════
   ...
   🐛 ما تُظهره لقطة المالك: اسم الغرفة انكمش إلى حرف واحد («ا»)
   والسطر الثاني إلى «1 م...»، وزرّ الرجوع مقصوص خارج الحافّة اليمنى.

   سببان متراكبان:
   ١) كل الأزرار `shrink-0` وعمود الهويّة `min-w-0` بلا `flex-1`،
      فالأزرار تأخذ ما تشاء ويُسحق الاسم إلى الصفر.
   ٢) `overflow-x-auto` جعل الفائض تمريراً، والتمرير في RTL يبدأ من
      الطرف الخطأ فيُقصّ أوّل عنصر (انظر `rtl-scroll.ts`).

   ولذلك لا يظهر العطب عند المُنضمّ: أزراره أقلّ فلا يفيض الشريط.
   وهذا بالضبط ما يعنيه «اختبر Owner وParticipant لا Owner فقط».

   الشريط الآن **لا يفيض أصلاً**: صفّ واحد بلا تمرير، الأزرار
   `shrink-0`، والهويّة `min-w-0 flex-1` — فهي وحدها من يتقلّص، وهو
   الصحيح لأنّ الاسم قابل للقصّ بنقاط والزرّ لا.
   ════════════════════════════════════════════════════════════ */
export function WorkspaceBar({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div className={`flex h-[3.25rem] w-full min-w-0 shrink-0 items-center gap-1.5 overflow-hidden border-b border-[var(--bz-line)] bg-[var(--bz-surface,#fff)] px-2.5 sm:gap-2 sm:px-3.5 ${className}`}>
      {children}
    </div>
  );
}
