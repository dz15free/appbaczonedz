...
export async function POST(req: NextRequest) {
  if (!KEY) {
    return Response.json({ error: "AiBot غير مُعدّ: مفتاح GEMINI_API_KEY مفقود." }, { status: 500 });
  }
  ...
  try {
    ...
    for (let attempt = 0; attempt < MODELS.length; attempt++) {
      ...
      if (res.ok) {
        ...
        console.error("[AiBot] Gemini error:", model, res.status, raw.slice(0, 120));
    ...
    console.error("[AiBot] fetch error:", e);
