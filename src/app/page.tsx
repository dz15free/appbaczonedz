...
export default function LandingPage() {
  ...
  return (
    <main className="relative overflow-hidden">
      {/* الشريط العلوي */}
      <header className="bz-glass sticky top-0 z-50 flex items-center justify-between gap-2 px-3 py-2.5 sm:px-5 sm:py-3">
        <Link href="/" className="flex min-w-0 items-center gap-2 sm:gap-2.5">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={s.logoUrl || "/icon.svg"} alt={s.siteName || "BacZoneDZ"} className="h-9 w-9 shrink-0 rounded-xl object-contain sm:h-10 sm:w-10" />
          <span className="truncate font-display text-lg font-extrabold sm:text-xl">{s.siteName || "BacZoneDZ"}</span>
        </Link>
